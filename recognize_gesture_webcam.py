	...
	while True:
		...
		if len(contours) > 0:
			...
			if cv2.contourArea(contour) > 10000:
				...
				cv2.imshow("boundary maker",save_img)
				if pred_probab*100 > 80:

					text=get_pred_text_from_db(pred_class)
					cv2.putText(img,text,(50,150), font,4,(0,125,155),2) 
										# if (pred_class!=None):
					# 	if(temp==0):
					# 		previouslabel=pred_class
					# 	if(previouslabel==pred_class):
					# 		previouslabel=pred_class
					# 		temp+=1
					# 	else:
					# 		temp=0
					...
					# 		if(pred_class==15):
					# 			words=re.split(" +",text_final)
					# 			words.pop()
					# 			text=" ".join(words)
					# 		print(text_final)
					# 		cv2.putText(img,text,(50,150), font,8,(0,125,155),2)  # displaying the predicted letter on the main screen
					# 		cv2.putText(img,text_final,(50,450), font,3,(0,0,255),2)
